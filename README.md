...
![Dashboard](https://github.com/bhawna407/Crisis-in-Quick-Bites-Food-Startup-/blob/main/image/quick%20bites%20pbix%202.png)
...
- **Fix delivery SLA in Delhi & Hyderabad first** — SLA compliance is at 12.2% (from 47.4%) and is the root cause driving cancellations, ratings, and churn. 87.8% of deliveries are late, 68% of those by 10+ min, concentrated in these two cities (~18 min avg delay). This is the single highest-leverage lever — every other metric (cancel rate, sentiment, revenue) traces back to it.

- **Run a public, visible food-safety trust campaign** — "Terrible hygiene" reviews carry the worst sentiment/rating (-0.90, 1.20) and hygiene/food-safety complaints dominate negative review terms. Sentiment fell 133% (faster than rating's 44.5% drop) and stayed negative even as ratings flattened — customers won't return on delivery fixes alone; they need proof the original safety issue is resolved.

- **Launch a segmented win-back for lost loyal customers before broad reacquisition spend** — only 37% of loyal customers retained (58→26) and high-value customer orders fell ~93% to near-zero. Acquiring new users now (CAC rising, "New" customer share jumped to ~45-54% post-crisis) is inefficient — retention economics won't work until trust metrics recover, so fix loyalty first.

- **Prioritize Bengaluru & Mumbai for cancellation control, Hyderabad/Chennai for delivery-speed fixes** — Bengaluru and Mumbai show the steepest cancellation spikes (>110%) on ~70% order drops (highest revenue-at-risk markets), while Hyderabad/Chennai have a distinct delivery-speed problem (~18 min delay) independent of volume — these need different interventions, not a blanket city-wide fix.

- **Audit and differentiate restaurant partners into "recoverable" vs "at-risk" before blanket promos** — Urban Kitchen Zone (-85.1% orders) and Flavours of Tandoor Central (-50.2% rating) are outliers even within an already-bad top-15 list; sentiment decline (-109% to -153%) outpaces rating decline across the board, meaning restaurant-level partner health checks (supply/quality) are needed, not platform-wide discounting.
